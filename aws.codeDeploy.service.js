...
module.exports = class CodeDeployService {
  constructor({ accessKeyId, secretAccessKey, region }) {
    ...
    this.aws = {
      ...
      describeAutoScalingGroups: (
        promisify(this.autoScaling.describeAutoScalingGroups).bind(this.autoScaling)
      ),
  ...
  async createApplication({ name, tags }) {
    if (!name) {
      throw new Error("Must provide application name!");
    }
  ...
  async createDeploymentGroup({
    application, name, serviceRole, autoScalingGroups, ec2TagFilters, onPremisesInstanceTagFilters,
    deploymentConfig, loadBalancingType, loadBalancer, elbTargetGroup,
  }) {
    if (!application || !name || !serviceRole) {
      throw new Error("Didn't provide all required parameters.");
    }
    if (loadBalancingType === "Classic" && !loadBalancer) {
      throw new Error("Must provide a loadBalancer if specified 'Classic' load balancing.");
    }
    ...
    const params = {
      ...
      onPremisesInstanceTagFilters: (
        onPremisesInstanceTagFilters ? onPremisesInstanceTagFilters.map(addTagType) : undefined
      ),
      autoScalingGroups,
    };
  ...
  async createDeploymentConfig({ name, minHealthyHostsNum, minHealthyHostsPercent }) {
    if (!name || !(minHealthyHostsNum || minHealthyHostsPercent)) {
      throw new Error("Didn't provide all required parameters.");
    }
    if (minHealthyHostsPercent && minHealthyHostsNum) {
  ...
  async listAll(funcName, outputName, params = {}) {
    try {
      let result = await this.aws[funcName](params);
  ...
  async listApps({ nextToken, listAll }) {
    if (listAll) {
      return this.listAll("listApplications", "applications");
    }
